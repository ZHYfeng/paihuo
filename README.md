...
11. **可选交互式终端**：手工任务默认仍是批处理；Pi、OMP、OpenCode、Claude Code 与 Codex 均可选择原生交互终端，任务会保留真实 TTY。点击终端即可直接输入，Tab、方向键与组合键会原样传给当前 CLI；输入 `/exit`（或点击结束会话）后再结算。每个交互会话仍唯一绑定任务 ID，并与批处理任务共同占用角色的并发槽位。定时任务始终保持批处理。
