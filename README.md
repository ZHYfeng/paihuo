...
12. **常驻会话（交互式工作区）**：会话页提供复杂问题的常驻多轮协作工作区（独立 worktree），完成时点「交付」转为任务走审批合并流程。**交互式会话只支持 pi / omp 角色**（RPC 消息流：思考/文本/工具卡片实时展示），支持挂起恢复与空闲自动挂起；opencode / claude / codex 无结构化消息通道，仅批处理执行。
