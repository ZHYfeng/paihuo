...
5. **Skills**：技能库（可定向添加，或递归扫描一个目录自动发现全部含 `SKILL.md` 的 skills；复制到工作目录登记，按名称/描述勾选）+ Pi Extensions 管理（pi install/list/remove 的 Web 封装）；已安装扩展包可在创建/编辑 Pi 角色时按需勾选
...
11. **可选交互式终端**：手工任务默认仍是批处理；**交互式执行只支持 Pi 与 OMP 角色**（两者有 RPC 消息流通道），任务会保留真实 TTY。点击终端即可直接输入，Tab、方向键与组合键会原样传给当前 CLI；输入退出命令（pi 为 `/quit`、omp 为 `/exit`，或点击结束会话）后再结算。每个交互会话仍唯一绑定任务 ID，并与批处理任务共同占用角色的并发槽位。定时任务始终保持批处理。
12. **常驻会话（交互式工作区）**：会话页提供复杂问题的常驻多轮协作工作区（独立 worktree），输入区可将提示词模板插入当前草稿，完成时点「交付」转为任务走审批合并流程。**交互式会话只支持 pi / omp 角色**（RPC 消息流：思考/文本/工具卡片实时展示），支持挂起恢复与空闲自动挂起；opencode / claude / codex 无结构化消息通道，仅批处理执行。
13. **Schedules / History / Settings**：定时任务、历史筛选批量管理、数据/工作空间保留策略
...
| pi | model、system_prompt、instructions、thinking、skills、**extensions**（角色级 `--no-extensions` + `--extension`）、provider、tools、exclude_tools、models_cycle、extra_args、env | [pi.dev/docs](https://pi.dev/docs) |
