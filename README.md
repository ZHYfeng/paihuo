...
2. **看板 → 新建任务**（顶栏右上角）：填标题和提示词，指派给角色，选权限模式；看板只展示活跃任务（排队/执行中/待审批）
3. agent 自动领取执行；点击任务卡片打开详情，**对话面板以终端样式实时滚动**（时间戳 + 流分色，可全屏、可复制）
4. 完成后任务进入**历史页**：表格视图按角色/状态/天数筛选，可勾选批量删除、单条重试；点标题随时回看该任务的完整终端对话
5. 满意的任务可"保存为模板"复用提示词
