...
export function SchedulesPage() {
  ...
  return <>
    ...
    {schedules.isLoading ? <Spinner /> : schedules.data?.length ? <Card className="overflow-hidden p-0"><table className="w-full text-sm"><thead><tr className="border-b border-line text-left text-xs text-faint">
      ...
      <td className="px-4 py-2.5"><span className="inline-flex gap-1.5"><Button size="sm" variant="ghost" onClick={() => openEditor(item)}><Pencil size={14} />编辑</Button><Button size="sm" variant="danger" onClick={() => confirm(`删除“${item.name}”？`) && remove.mutate(item)}><Trash2 size={14} /></Button></span></td>
    ...
    <Dialog open={draft !== null} onOpenChange={open => !open && setDraft(null)} title={draft?.id ? "编辑定时任务" : "新建定时任务"} wide>{draft && <form className="grid gap-4" onSubmit={(e: FormEvent) => { e.preventDefault(); save.mutate(draft); }}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="mt-7 flex min-h-11 items-center gap-3 rounded-xl border border-line bg-elevated px-3 text-sm"><input type="checkbox" checked={draft.block_on_failure ?? true} onChange={e => setDraft({ ...draft, block_on_failure: e.target.checked })} />失败后阻塞后续任务</label>
      </div>
