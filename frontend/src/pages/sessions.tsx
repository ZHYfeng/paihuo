...
import { CirclePlus, Copy, Pause, Play, Send, Square, Trash2, Truck } from "lucide-react";
...
export function SessionsPage() {
  ...
  return <>
    ...
    <Card className="mb-5 flex flex-col gap-3 p-3 sm:flex-row sm:items-center">
      ...
      <div className="flex flex-wrap gap-1 rounded-xl border border-line bg-elevated p-0.5">{FILTERS.map(([value, label]) => <button key={value} className={`rounded-[10px] px-3 py-1.5 text-sm ${filter === value ? "bg-surface font-semibold text-ink shadow-sm" : "text-muted"}`} onClick={() => setFilter(value)}>{label}</button>)}</div>
      <span className="text-sm text-muted sm:ml-auto">{filtered.length} 个会话</span>
    </Card>
    {sessions.isLoading ? <Spinner /> : filtered.length ? <div className="grid gap-3 lg:grid-cols-2">{filtered.map(item => <Link key={item.id} to={`/sessions/${item.id}`} className="rounded-xl border border-line bg-surface p-4 shadow-card transition hover:border-brand/35 hover:bg-hover focus-visible:ring-2 focus-visible:ring-focus"><div className="flex items-center gap-2"><span className="text-xs text-faint">#{item.id}</span><h2 className="truncate font-semibold">{item.title}</h2><Badge tone={sessionTone[item.status] || "neutral"}>{sessionLabel[item.status] || item.status}</Badge></div><div className="mt-3 flex flex-wrap gap-2 text-sm text-muted"><span>{item.role_name}</span>{item.project_name && <span>· {item.project_name}</span>}<span>· {item.message_count} 条消息</span>{item.task_id && <span>· 任务 #{item.task_id}</span>}<span className="ml-auto">{formatTime(item.updated_at)}</span></div></Link>)}</div> : <Empty title={filter === "all" ? "还没有会话" : "没有匹配的会话"} copy="会话保存完整的结构化对话，可在后续交付为任务。" />}
...
export function SessionDetailPage() {
  ...
  const templates = useQuery({ queryKey: ["templates"], queryFn: () => api<TaskTemplate[]>("/templates"), enabled: session.data?.status === "active" });
  ...
  const prompt = useMutation({ mutationFn: () => api(`/sessions/${id}/prompt`, { method: "POST", body: { message, streaming_behavior: behavior } }), onSuccess: () => { setMessage(""); qc.invalidateQueries({ queryKey: ["sessions", id, "transcript"] }); }, onError: error => toast(error instanceof Error ? error.message : "发送失败", "bad") });
  ...
  return <>
    ...
    {item.status === "active" && <form className="sticky bottom-4 mt-4 flex gap-3 rounded-xl border border-line bg-surface/95 p-3 shadow-pop backdrop-blur" onSubmit={e => { e.preventDefault(); send(); }}>
