...
export function AppShell() {
  ...
  const nav = <>
    <Link to="/" className="flex items-center gap-2.5 px-2.5 py-3" onClick={() => setMobileOpen(false)}>
      ...
      {groups.map(group => <div key={group.label}>
        ...
        <div className="grid gap-0.5">
          {group.items.map(([to, label, Icon]) => <NavLink key={to} to={to} end={to === "/"} onClick={() => setMobileOpen(false)} className={({ isActive }) => cn(
            "group flex min-h-9 items-center gap-2 rounded-lg px-2.5 text-[13px] font-medium text-muted transition-colors focus-visible:ring-2 focus-visible:ring-focus",
    ...
    <div className="mt-auto grid gap-2.5 px-2.5 pb-3 pt-4">
      ...
      <div className="flex gap-2">
        <button onClick={() => setDark(value => !value)} className="grid size-9 place-items-center rounded-lg border border-line text-muted hover:bg-hover focus-visible:ring-2 focus-visible:ring-focus" aria-label={dark ? "切换到亮色" : "切换到暗色"}>{dark ? <Sun size={16} /> : <Moon size={16} />}</button>
        <form action="/logout" method="post" className="flex-1"><button className="h-9 w-full rounded-lg border border-line text-[13px] text-muted hover:bg-hover hover:text-ink">退出</button></form>
      </div>
    </div>
  </>;
  ...
  return <div className="min-h-screen bg-canvas text-ink">
    ...
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-52 flex-col border-r border-line bg-surface/95 p-2.5 backdrop-blur lg:flex">{nav}</aside>
