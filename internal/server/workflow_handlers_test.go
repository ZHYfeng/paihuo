...
// 创建工作流即冻结；启动 Run 绑定具体项目并原子实例化节点任务。
func TestWorkflowCreateFreezesAndStartBindsProject(t *testing.T) {
	root := t.TempDir()
	...
	// 1. 创建即冻结：201 + adopted + spec_hash，且定义本身不绑定项目。
	...
	if out["status"] != workflow.WorkflowStatusFrozen {
		t.Fatalf("status=%v, want frozen", out["status"])
	}
	if out["spec_hash"] == "" {
