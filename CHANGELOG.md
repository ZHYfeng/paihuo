...
- **终端式会话（codex/claude）中文/宽字符错乱修复**：帧同步的行宽截断此前按 JS 字符串长度（`slice(0, cols)`），而 CJK/全角/emoji 在终端占 2 列——含中文的行截断后实际超宽，xterm 自动折行导致后续行整体错位（交互式对话的中文消息/代码注释必触发）。现在按显示宽度截断（宽字符表：CJK 统一表意、全角符号、Hangul、emoji 等占 2 列，不拆宽字符），整帧/行级 diff 两条写入路径统一走宽度感知截断；行级 diff 后光标归位到帧最后一行内容末尾（TUI 输入框通常在底部），不再跳到左上角。
