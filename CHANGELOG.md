...
### Added
- **模板页移动端改为分区卡片布局**：模板管理列表此前在 ≤720px 仍是 760px 宽的桌面表格，只能横向滚动查看，名称/内容预览被压缩、操作按钮拥挤。现在与定时任务/角色/技能列表一致转为纵向卡片：名称独占首行，角色/创建时间作为带标签的元信息并排，内容预览整行换行展示（最多 3 行），编辑/新建任务/删除按钮平铺为 44px 触控目标。
- **会话顶部进度条改为显示实际阅读位置**：此前 conversation rail 把「已加载条数 ÷ 总条数」直接当成位置，打开长会话即使位于最新消息底部也会错误显示为 10% 等加载比例，且上下滚动时进度不变。现在结合未加载历史偏移与消息区 `scrollTop` 计算全文阅读位置：首屏尾页覆盖正确的末段区间，滚到最新消息稳定为 100%，加载全部历史后覆盖 0%–100%；分页插入和实时消息导致高度变化时会自动校准，并补齐 `progressbar` ARIA 数值。
- **新增「根据对话创建任务」默认模板**：新安装与已有数据库在升级后都会一次性获得该模板，便于让 agent 根据当前对话，在 PaiHuo 的当前项目中创建合适任务；后续手动编辑或删除不会在重启时被覆盖或恢复。
- **OMP 会话消息改用完整、安全的 GFM 渲染**：原正则版 `md()` 不识别段落、有序/嵌套列表、任务列表和表格，连续无序列表还会生成游离在 `<ul>` 外的 `<li>`；助手文本容器又缺少 `.ph-md` 类，导致表格/标题/代码块即使解析也吃不到排版。现在用 `marked` 按 GFM 语义解析，通过 DOMPurify 显式白名单清洗后渲染，补齐段落/列表/表格/分隔线/删除线/任务复选框样式与窄屏表格横向滚动；代码块复制按钮保留，危险 HTML 和 `javascript:`/`data:` 等链接协议被移除或降级为 `#`。
...
- **默认模板「根据对话创建任务」补全建任务操作说明**：正文从一句「请根据当前对话的内容，在 paihuo 系统中的当前项目中创建合适的任务」扩展为完整手册——本机实例默认地址 `http://127.0.0.1:8080`、`POST /login` 用 `PAIHUO_TOKEN` 换取会话 cookie、`GET /api/agents` / `GET /api/projects` 查询角色与项目 ID、`POST /api/tasks` 的完整 curl 示例与逐字段说明（title / body / agent_id / project_id / perm / run_mode / concurrent / dependency_mode / depends_on / block_on_failure / parent_id），并提示按 201 响应确认创建结果、4xx 按 `error` 字段修正。已应用 v1 迁移的数据库通过新迁移（`default-template-create-tasks-v2`）只升级**未被用户编辑过**的默认模板（正文仍等于 v1 原文才更新）；用户改过或删过的一律保留原状，仍遵循「重启不覆盖不恢复」约定。
