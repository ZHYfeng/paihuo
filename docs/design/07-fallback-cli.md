...
> 状态：已删除 · 更新：2026-08-09
> **已删除（2026-08-09）**：产品决策为「交互式会话只支持 pi / omp」，S5 终端
> 降级通道已彻底移除——`termProc` 实现、`/api/sessions/{id}/terminal/*` 端点、
> `BuildInteractiveArgs` 与前端终端式会话组件全部删除，会话一律为 pi/omp 消息流
> 视图；遗留的非 pi/omp 会话无法启动（后端显式拒绝）。本文档保留为历史设计记录。
